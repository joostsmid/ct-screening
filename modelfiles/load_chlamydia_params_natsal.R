...
load(file.path(path_data,"nat2.Rda"))
load(file.path(path_data,"nat3.Rda"))

##############################################################################################################################
### redefine age classes to single years age bands (for smoothing age mixing data)
##############################################################################################################################

age.classes.mainmodel <- age.classes
age.classes <- 13:63
age.classes.partner <- 11:70
...
source(file.path(path_in,"loss_virginity.R"))
vl.unsmooth2 <- prob.virgin(nat2, age.classes)
vl.unsmooth3 <- prob.virgin(nat3, age.classes)

#aggregate
vl.meannat23 <- vl.unsmooth3
for (k in 1:2){
  for (a in 1:n.age.classes){
    if (!is.na(vl.unsmooth2[k,a])){
      vl.meannat23[k,a] <- (vl.meannat23[k,a] + vl.unsmooth2[k,a])/2
    }
  }
}

vl.unsmooth <- vl.meannat23

vl <- prob.virgin.smooth(vl.meannat23)

#############################################################################################################################
### acquisition rate of new sexual partners per year, activity class and age
##############################################################################################################################

### generate contact structure
source(file.path(path_in,"partner_acquisition.R"))

#combine data from natsal 2 and 3
contact.rates <- new.contact.nmbs.nat2_3combined(nJ, age.classes)

ct <- contact.rates[[1]]

### replace all NA's by averages of the adjacent values
ct <- replace.na.by.averaging(ct)

### smoothen the results (apply smoothing spline of 6 degrees of freedom)
ct.smooth <- smooth.est.age(ct, df=6) 
ct.unsmooth <- ct
ct <- ct.smooth

##############################################################################################################################
### probability with which the virgins go into the different classes of sexual activity
##############################################################################################################################

fold <- contact.rates$risk.cat
save(fold,file=file.path(path_data,"fold.Rda"))
...
source(file.path(path_in,"load_mixing_parameters.R"))
rownames(mm) <- paste0("age",age.class.groups); colnames(mm) <- paste0("age",age.class.groups)
rownames(mf) <- paste0("age",age.class.groups); colnames(mf) <- paste0("age",age.class.groups)

##############################################################################################################################
### reaggregate age classes
##############################################################################################################################

age.classes <- age.classes.mainmodel
n.age.classes <- length(age.classes)-1
age.class.groups <- age.classes[1:n.age.classes]
...
vlshort <- to.tensor(rep(0,n.age.classes*2), dims = list(sex=c("M","F"), age=paste0('age', age.class.groups))) #we suppose that there is no mortality
for (k in 1:n.age.classes){
  lb_age <- match(age.classes[k] ,as.numeric(gsub("age", "", dimnames(vl)$age)) )
  ub_age <- match(age.classes[k+1] ,as.numeric(gsub("age", "", dimnames(vl)$age)) )-1
  vlshort[1,k] <- mean(as.numeric(vl[1,lb_age:ub_age]))
  vlshort[2,k] <- mean(as.numeric(vl[2,lb_age:ub_age]))
}

vl <- vlshort
save(vl,file=file.path(path_data,"vl.Rda"))

##############################################################################################################################
### acquisition rate of new sexual partners per year, activity class and age
##############################################################################################################################

matchages <- match(age.classes ,as.numeric(gsub("age", "", dimnames(ct)$age)) )


ctshort <- to.tensor(rep(NA,nJ*(n.age.classes)*2), dims = list(sex=c("M","F"), j=paste0('j',1:nJ), age=paste0('age', age.classes[1:(n.age.classes)])) )
for (k in 1:n.age.classes){
  for (j in 1:nJ){
    ctshort[1,j,k] <- mean(as.vector(ct[1,j,matchages[k]:(matchages[k+1]-1)]))
    ctshort[2,j,k] <- mean(as.vector(ct[2,j,matchages[k]:(matchages[k+1]-1)]))
  }
}

ct <- ctshort
save(ct,file=file.path(path_data,"ct.Rda"))

##############################################################################################################################
### age related sexual contact preference matrix
##############################################################################################################################

matchages <- match(age.classes ,as.numeric(gsub("age", "", rownames(mm))) )

mm <- mm[matchages[1]:(tail(matchages,n=1)-1), matchages[1]:(tail(matchages,n=1)-1)]
mm <- sweep(mm, 1, rowSums(mm), FUN="/")
mf <- mf[matchages[1]:(tail(matchages,n=1)-1), matchages[1]:(tail(matchages,n=1)-1)]
mf <- sweep(mf, 1, rowSums(mf), FUN="/")

mmshort <- matrix(NA, nrow=n.age.classes, ncol=n.age.classes)
mfshort <- matrix(NA, nrow=n.age.classes, ncol=n.age.classes)
for (j in 1:n.age.classes){
  for (k in 1:n.age.classes){
    mmshort[j,k] <- sum(mm[(matchages[j]-matchages[1]+1):(matchages[j+1]-matchages[1]), (matchages[k]-matchages[1]+1):(matchages[k+1]-matchages[1])])
    mfshort[j,k] <- sum(mf[(matchages[j]-matchages[1]+1):(matchages[j+1]-matchages[1]), (matchages[k]-matchages[1]+1):(matchages[k+1]-matchages[1])])
  }
}

mmshort <- sweep(mmshort, 1, rowSums(mmshort), FUN="/") #normalize across rows
mfshort <- sweep(mfshort, 1, rowSums(mfshort), FUN="/") #normalize across rows
mm <- mmshort
mf <- mfshort
rownames(mm) <- paste0("age",age.class.groups); colnames(mm) <- paste0("age",age.class.groups)
rownames(mf) <- paste0("age",age.class.groups); colnames(mf) <- paste0("age",age.class.groups)

save(mm,file=file.path(path_data,"mm.Rda"))
save(mf,file=file.path(path_data,"mf.Rda"))
